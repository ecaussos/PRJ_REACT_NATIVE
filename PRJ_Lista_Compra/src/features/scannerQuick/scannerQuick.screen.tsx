// features/quickScanner/quickScanner.screen.tsx
...
export function QuickScannerScreen(): React.JSX.Element {
  // Variáveis de estado
  ...
  const handleClose = () => {
    ...
    router.back();            // Retornar a última tela acessada
  };

  // Processamento do código de barra
  const handleScanSuccess = async (barcode: string) => {  // função assíncrona (async) recebe o código de barra (barcode)
    // Bloco tratamento: Try: Tente | Catch: Capture (Erro)
    try {
      // 1. Valida se o produto existe no banco
      const product = await ProductModelInstance.checkBarcode(barcode.trim());
      ...
      await BuyListModelInstance.create({
